...
## My custom bindings
 ...
 `go`  Open link in new tab and switch to it
 `gx` Close tabs to the left of current tab 
 `gX` Close tabs to the right of current tab 
 `gd` Download contents from the link
 `gw` Open link in Incognito window
 `zX`  Close all other tabs except the current one 
